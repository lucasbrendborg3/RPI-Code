...
struct Axis {
    ...
    double gear_ratio_ = 1.0;  // default to 1.0 (no gearing)
...
CallbackReturn ODriveHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
    ...
    for (auto& joint : info_.joints) {
        Axis axis(&can_intf_, std::stoi(joint.parameters.at("node_id")));
        if (joint.parameters.find("gear_ratio") != joint.parameters.end()) {
            axis.gear_ratio_ = std::stod(joint.parameters.at("gear_ratio"));
        }
